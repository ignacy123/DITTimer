...
public class StatsWindow implements Initializable {
    ...
    private void load() {
        ...
        for (int j = Math.max(morszczuk.getListOfSolves(type).size() - count, 0), i = 0; j < morszczuk.getListOfSolves(type).size(); j++, i++) {
            ...
            int in = (int) a.getTime().getTime();
            if(a.getState().equals(State.DNF)) {
                in=0;
            }
            data.getData().add(new XYChart.Data<>(Integer.toString(i + 1), in));
        }
        data.setName("all");
        ...
        for (int j = Math.max(morszczuk.getListAvg(type, AVG.Ao5).size() - count, 0), i = 0; j < morszczuk.getListAvg(type, AVG.Ao5).size(); j++, i++) {
            ...
            int in = (int) avg.getAVG().getTime();
            if(avg.isDNF()) {
                in=0;
            }
            data2.getData().add(new XYChart.Data<>(Integer.toString(i + 1), in));
        }
        data2.setName("Ao5");
        ...
        for (int j = Math.max(morszczuk.getListAvg(type, AVG.Ao12).size() - count, 0), i = 0; j < morszczuk.getListAvg(type, AVG.Ao12).size(); j++, i++) {
            ...
            data3.getData().add(new XYChart.Data<>(Integer.toString(i + 1), in));
        }
        data3.setName("Ao12");
    ...
    public void setOw(ObservableWrapper ow) {
        ...
        morszczuk.getCubeCurrType().addListener((ListChangeListener<CubeType>) change -> {
        ...
        for (CubeType type : types) {
            morszczuk.getListAvg(type, AVG.Ao5).addListener((ListChangeListener<AVGwrapper>) change -> load());
        }
        load();
    }
